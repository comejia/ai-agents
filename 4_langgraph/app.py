...
with gr.Blocks(title="Sidekick", theme=gr.themes.Default(primary_hue="emerald")) as ui:
    ...
    with gr.Group():
        with gr.Row():
            message = gr.Textbox(
                show_label=False, placeholder="Tu solicitud al Sidekick"
            )
        with gr.Row():
            success_criteria = gr.Textbox(
                show_label=False, placeholder="¿Cuáles son tus criterios de éxito?"
            )
    with gr.Row():
        reset_button = gr.Button("Reiniciar", variant="stop")
        go_button = gr.Button("Enviar", variant="primary")
